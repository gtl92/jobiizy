(() => {
  ...
  Jobiizy.openApplyPopup = function ({ title, content , jobData }) {
    document.getElementById("jobiizy-apply-modal")?.remove();

    const modal = document.createElement("div");
    modal.id = "jobiizy-apply-modal";
	const dataForCopy = {
    ...
    modal.innerHTML = `
      <div class="jobiizy-modal-backdrop" data-close></div>
...
iframe.src = url.includes('?')
  ? url + '&jobiizy_preview=1'
  : url + '?jobiizy_preview=1';

	modal.removeAttribute('hidden'); // 🔥 IMPORTANT
