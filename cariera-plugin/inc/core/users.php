...
class Users {
	...
	public function registration_validation( $username, $email, $password, $privacy_policy, $user_role ) {
		...
		if ( class_exists( 'Cariera_Core\Extensions\Recaptcha\Recaptcha' ) && \Cariera_Core\Extensions\Recaptcha\Recaptcha::is_recaptcha_enabled() && $registration_captcha ) {
			...
			$is_recaptcha_valid = array_key_exists( 'g-recaptcha-response', $_POST ) ? \Cariera_Core\Extensions\Recaptcha\Recaptcha::is_recaptcha_valid( sanitize_text_field( wp_unslash( $_POST['g-recaptcha-response'] ) ) ) : false;
			if ( ! $is_recaptcha_valid ) {
				$reg_errors->add( 'field', esc_html__( 'reCAPTCHA is a required field', 'cariera-core' ) );
			}
		}
		...
		if ( $password_length > strlen( $password ) ) {
			// translators: %s is the password length.
	...
	public function register_process() {
		...
		if ( get_option( 'cariera_register_hide_username' ) ) {
			$reg_email    = explode( '@', $_POST['register_email'] ); // phpcs:ignore
			...
			$reg_username = sanitize_user( $_POST['register_username'] ); // phpcs:ignore
		}

		// Check Privacy Policy if enabled.
		if ( 1 === intval( get_option( 'cariera_register_privacy_policy' ) ) ) {
			$privacy_policy = isset( $_POST['privacy_policy'] ) ? sanitize_text_field( $_POST['privacy_policy'] ) : ''; // phpcs:ignore
		} else {
			$privacy_policy = 1;
		}
		...
		$this->registration_validation( $reg_username, $_POST['register_email'], $_POST['register_password'], $privacy_policy, $user_role ); // phpcs:ignore

		// If there are no errors during registration.
		if ( 1 > count( $reg_errors->get_error_messages() ) ) {

			$username = $reg_username;
			$email    = sanitize_email( $_POST['register_email'] ); // phpcs:ignore
			$password = sanitize_text_field( $_POST['register_password'] ); // phpcs:ignore
	...
	public function forgot_pass_process() {
		...
		if ( class_exists( 'Cariera_Core\Extensions\Recaptcha\Recaptcha' ) && \Cariera_Core\Extensions\Recaptcha\Recaptcha::is_recaptcha_enabled() && $forgotpass_captcha ) {
			$is_recaptcha_valid = array_key_exists( 'g-recaptcha-response', $_POST ) ? \Cariera_Core\Extensions\Recaptcha\Recaptcha::is_recaptcha_valid( sanitize_text_field( $_POST['g-recaptcha-response'] ) ) : false; // phpcs:ignore
		...
		if ( empty( $error ) ) {
			...
			if ( $update_user ) {
				...
				<tr><td class="h2"><?php printf( esc_html__( 'Hello %s,', 'cariera-core' ), $user->user_login ); ?></td></tr>
				<tr><td><?php esc_html_e( 'Your password has been resetted successfully. You can log in on your account with the newly generated password provided below.', 'cariera-core' ); ?></td></tr>
				<tr><td style="padding-top: 15px;"><?php printf( esc_html__( 'Your new password is: %s', 'cariera-core' ), $random_password ); ?></td></tr>
				...
				$success = esc_html__( 'Go to your inbox or spam/junk and get your new generated password.', 'cariera-core' );
			} else {
				$error = esc_html__( 'Something went wrong while updating your account.', 'cariera-core' );
			}
		}
	...
	public static function status_column( $val, $column_name, $user_id ) {
		switch ( $column_name ) {
			case 'user_status':
				...
				if ( 'approved' === $status ) {
					$status_i18n = esc_html__( 'approved', 'cariera-core' );
				} elseif ( 'denied' === $status ) {
					$status_i18n = esc_html__( 'denied', 'cariera-core' );
				} elseif ( 'pending' === $status ) {
					$status_i18n = esc_html__( 'pending', 'cariera-core' );
				}
				return $status_i18n;
				break;
	...
	public static function process_update_user_action() {
		if ( isset( $_GET['action'] ) && in_array( $_GET['action'], [ 'approve', 'deny' ], true ) && ! isset( $_GET['new_role'] ) ) {
			...
			$user   = absint( $_GET['user'] ); // phpcs:ignore
	...
	public static function frontend_approve_user() {
		...
		if ( is_object( $post ) ) {
			if ( strpos( $post->post_content, '[cariera_approve_user]' ) !== false ) {
				...
				$user_id = isset( $_GET['user_id'] ) ? sanitize_text_field( wp_unslash( $_GET['user_id'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$code    = isset( $_GET['approve-key'] ) ? sanitize_text_field( wp_unslash( $_GET['approve-key'] ) ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function extra_profile_fields( $user ) {
		?>
		<h3><?php esc_html_e( 'Cariera Avatar', 'cariera-core' ); ?></h3>
		<?php wp_enqueue_media(); ?>

		<table class="form-table">
			<tr>
				<th><label for="image"><?php esc_html_e( 'Avatar', 'cariera-core' ); ?></label></th>
				<td>
					<?php
					...
					<input type='button' class="cariera-user-avatar button-primary" value="<?php esc_html_e( 'Upload Image', 'cariera-core' ); ?>" id="uploadimage"/><br />
					<span class="description"><?php esc_html_e( 'This avatar will be displayed instead of default one', 'cariera-core' ); ?></span>
				</td>
			</tr>
		</table>
		<?php
	...
	public function save_extra_profile_fields( $user_id ) {
		...
		if ( isset( $_POST['cariera_avatar_id'] ) ) {
			update_user_meta( $user_id, 'cariera_avatar_id', sanitize_text_field( wp_unslash( $_POST['cariera_avatar_id'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		}
	}
	...
	public function change_user_details() {
		...
		$user_avatar_id = isset( $_POST['cariera_avatar_id'] ) ? sanitize_text_field( wp_unslash( $_POST['cariera_avatar_id'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$user_role      = isset( $_POST['cariera_user_role'] ) ? sanitize_text_field( wp_unslash( $_POST['cariera_user_role'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$first_name     = isset( $_POST['first-name'] ) ? sanitize_text_field( wp_unslash( $_POST['first-name'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$last_name      = isset( $_POST['last-name'] ) ? sanitize_text_field( wp_unslash( $_POST['last-name'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		if ( isset( $_POST['cariera_avatar_id'] ) ) { // phpcs:ignore
		...
		if ( isset( $_POST['first-name'] ) ) { // phpcs:ignore
			update_user_meta( $user_id, 'first_name', $first_name );
		}

		// Submitting the last name field.
		if ( isset( $_POST['last-name'] ) ) { // phpcs:ignore
			update_user_meta( $user_id, 'last_name', $last_name );
		}

		// Submitting the phone field.
		if ( isset( $_POST['phone'] ) ) { // phpcs:ignore
