...
class Cariera_Core_Promotions_WC {
	...
	public function __construct() {
		...
		// Dashboard Action.
		add_action( 'job_manager_job_dashboard_column_actions', [ $this, 'job_dashboard_action' ], 10, 2 );
		...
		echo '<div class="options_group show_if_job_promotion_package show_if_company_promotion_package show_if_resume_promotion_package">';
			woocommerce_wp_text_input(
				[
					...
					'label'             => esc_html__( 'Promotion duration', 'cariera-core' ),
					'description'       => esc_html__( 'The number of days that the listing will be promoted.', 'cariera-core' ),
	...
	public function save_product_settings( $post_id ) {
		...
		// Duration.
		if ( ! empty( $_POST['_promotion_duration'] ) ) { // phpcs:ignore
			update_post_meta( $post_id, '_promotion_duration', absint( $_POST['_promotion_duration'] ) ); // phpcs:ignore
		}
	}
	...
	public function display_listing_name_in_cart( $data, $cart_item ) {
		if ( isset( $cart_item['listing_id'] ) ) {
			$data[] = [
				'name'  => esc_html__( 'Listing', 'cariera-core' ),
	...
	public function woocommerce_thankyou( $order_id ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( $is_paid ) {
				...
				echo wpautop( sprintf( esc_html__( '"%s" has been promoted successfully.', 'cariera-core' ), get_the_title( $item['listing_id'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			} else {
				// translators: %s is the listing title.
				echo wpautop( sprintf( esc_html__( '"%s" will be promoted once the order is verified and completed.', 'cariera-core' ), get_the_title( $item['listing_id'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
	 ...
	 * @param array   $actions
	 */
	public function job_dashboard_action( $listing, $actions ) {
		...
		echo '<a href="#promotions-modal" class="job-dashboard-action-promote popup-with-zoom-anim" data-listing-id="' . esc_attr( $listing->ID ) . '"><i class="las la-bolt"></i><span>' . esc_html__( 'Promote', 'cariera-core' ) . '</span></a>';
	}
	...
	public function company_dashboard_action( $listing_id ) {
		...
		echo '<a href="#promotions-modal" class="company-dashboard-action-promote popup-with-zoom-anim" data-listing-id="' . esc_attr( $listing_id ) . '"><i class="las la-bolt"></i>' . esc_html__( 'Promote', 'cariera-core' ) . '</a>';
	}
	...
	public function resume_dashboard_action( $listing_id ) {
		...
		echo '<a href="#promotions-modal" class="candidate-dashboard-action-promote popup-with-zoom-anim" data-listing-id="' . esc_attr( $listing_id ) . '"><i class="las la-bolt"></i>' . esc_html__( 'Promote', 'cariera-core' ) . '</a>';
	}
	...
	public function get_job_promotions_modal() {
		...
		cariera_get_template(
			...
			[
				'title'    => esc_html__( 'Job Promotion', 'cariera-core' ),
				'products' => $products,
	...
	public function get_company_promotions_modal() {
		...
		cariera_get_template(
			'account/promotions/choose-promotion.php',
			[
				'title'    => esc_html__( 'Company Promotion', 'cariera-core' ),
				'products' => $products,
	...
	public function get_resume_promotions_modal() {
		...
		cariera_get_template(
			'account/promotions/choose-promotion.php',
			[
				'title'    => esc_html__( 'Resume Promotion', 'cariera-core' ),
				'products' => $products,
