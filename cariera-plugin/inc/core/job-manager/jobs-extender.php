	...
	public function job_listings_output( $atts ) {
		...
		if ( $atts['show_filters'] ) {
			...
			if ( ! $atts['show_pagination'] && $atts['show_more'] ) {
				echo '<div class="col-md-12 text-center"><a class="load_more_jobs btn btn-main btn-effect mt40" href="#" style="display:none;">' . esc_html__( 'Load more listings', 'cariera-core' ) . '</a></div>';
			}
		} else {
			...
			if ( $jobs->have_posts() || 1 === absint( $atts['sidebar_search'] ) ) {
				...
				if ( $jobs->found_posts > $atts['per_page'] && $atts['show_more'] ) {
					...
					if ( $atts['show_pagination'] ) {
						...
						echo '<div class="col-md-12 text-center"><a class="load_more_jobs btn btn-main btn-effect" href="#">' . esc_html__( 'Load more listings', 'cariera-core' ) . '</a></div>';
					}
				}
			} else {
