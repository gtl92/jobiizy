...
function cariera_user_can_submit_companies() {
	...
	return apply_filters( 'cariera_company_manager_user_can_submit_companies', $can_browse );
}
...
function cariera_user_can_view_contact_details( $company_id ) {
	...
	$key = get_post_meta( $company_id, 'share_link_key', true );
	if ( $key && ! empty( $_GET['key'] ) && $key == $_GET['key'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$can_view = true;
	}
